...
| **ARM node with libcamera via GbE → host**                   |                                    **~5–15 ms** |          **moderate** | Flexible and open, but adds software and buffering overhead                                                                                                    |
| **ARM node with custom lean V4L2 stack via GbE  → host**     |                                    **~3–10 ms** | **moderate to lower** | Reduced overhead compared to libcamera when optimized                                                                                                          |
| **ARM node with RTLinux + lean V4L2 stack via GbE → host**   |                                     **~2–8 ms** |   **low to moderate** | Further optimized for reduced scheduling delay and improved determinism                                                                                        |
| **ARM node with RTLinux + lean V4L2 stack via SFP+ → host**  |                                   **~1.5-4 ms** |               **low** | Lower protocol overhead and often better determinism than standard Ethernet/RJ45 setups, especially with DAC or fiber and a carefully tuned host path          |                                                                                |
